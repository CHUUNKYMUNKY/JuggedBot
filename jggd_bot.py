...
app = Client("jugged_g-host", api_id=api_id, api_hash=api_hash)
...
def deep_linked_level_1(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        track_url = "https://t.me/jugged/{}".format(
            random.choice(result_id)
        )
        for i in range(1):
            update.message.reply_text(
                "::*jugged* ""[🎧]({})".format(track_url),
                parse_mode=ParseMode.MARKDOWN
            )
...
def hashtag_handler(update: Update, context: CallbackContext) -> None:
    "Handle hashtags found in chat"
    for k, v in d.items():
...
def main():
    ...
    updater = Updater(
        "1296068786:AAGF4BERsuB-gjInko5DM_f4wuOCtuEmBRw", use_context=True
    )
    ...
    # dp.add_handler(MessageHandler(hashtag_handler, Filters.regex))
